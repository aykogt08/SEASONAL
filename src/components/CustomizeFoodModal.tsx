...
import { X, Upload, Trash2, Sparkles, Check } from "lucide-react";
...
export const CustomizeFoodModal: React.FC<CustomizeFoodModalProps> = ({
  ...
  const handleDelete = async () => {
    if (!confirm(`Are you sure you want to remove "${food.nameEn}" from your checklist?`)) return;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/35 backdrop-blur-xs animate-fade-in">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            ...
            <div className="flex items-center gap-3 p-2.5 bg-[#FAF8F5] rounded-2xl border border-[#EBE4DC]">
              ...
              <div className="text-xs text-[#8C7E75]">
                <p className="font-serif-title font-semibold text-[#3D322C]">
                  {iconMode === "preset" ? "Selected Icon" : "Custom Photo"}
                </p>
                <p className="text-[10px] text-[#3F9A90] mt-0.5">
                  {iconMode === "preset"
                    ? "Choose any default icon below"
                    : "Upload your custom food photo"}
                </p>
              </div>
            </div>
              ...
              <div className="mt-2.5">
                <div className="grid grid-cols-6 gap-1.5 max-h-36 overflow-y-auto p-1.5 bg-[#FAF8F5] rounded-2xl border border-[#EBE4DC]">
                  {ALL_PRESET_ICONS.map((preset) => {
                    const isSelected = selectedPresetKey === preset.key || iconUrl === getSvgDataUrl(preset.key);
              ...
              <div className="mt-2.5">
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-[#3F9A90]">
                    {isUploading ? "Converting & Compressing..." : "Click or Drop custom image"}
                  </span>
          ...
          {/* Action Buttons */}
          <div className="flex items-center justify-between pt-3 border-t border-[#F4EFEB]">
            <button
              type="button"
              onClick={handleDelete}
              disabled={isDeleting}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-red-500 hover:bg-red-50 transition-colors cursor-pointer"
            >
              <Trash2 className="w-3.5 h-3.5" />
              <span>{isDeleting ? "Removing..." : "Remove Item"}</span>
            </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="submit"
                disabled={isSaving}
                className="px-4 py-2 rounded-xl text-xs font-semibold bg-[#5DBBB0] hover:bg-[#3F9A90] text-white shadow-[0_4px_12px_-2px_rgba(93,187,176,0.35)] transition-all cursor-pointer"
              >
